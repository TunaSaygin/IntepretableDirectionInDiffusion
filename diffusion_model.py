...
class ModifiedUNet(UNet2DModel):
    ...
    def set_aux_params(
        ...
        """Set parameters for the auxiliary network"""
        self.aux_params = {
    ...
    def forward(self, x, timestep):
        ...
        def modified_mid_block_forward(hidden_states, temb=None):
            ...
            # Apply auxiliary network only during specific timesteps
            if (
                self.auxiliary_net is not None
                and self.aux_params
...
class CustomPretrainedDiffusionModel:
    ...
    def denoising_loop(
        ...
        if inject_aux:
            self.unet_edited.set_aux_params(**self.aux_params)
        ...
        for t in tqdm(self.scheduler.timesteps):
            ...
            if (
                ...
                noise_edited = checkpoint(
                    lambda x, t: self.unet_edited(x, t).sample, latent, t
                )
...
class DiffusionModel:
    ...
    def train_step(self, batch_size: int, M: int, magnitude: float = 1.0):
        ...
        # self.lpips_weight*loss_lpips + div_weight* div_loss
        ...
        return edits, origs, flat_dirs, loss.item(), loss_cls.item(), loss_shift.item(),0
    # , loss_lpips.item()
...
def visualize_all_direction_interpolations(dm, steps=7, max_mag=2.0, seed=42, save_dir = None):
    ...
    dm.aux_net.train()
def visualize_direction_across_latents(dm, direction_idx, num_samples=4, steps=7, max_mag=2.0, seed=None, save_dir=None):
    """
    For a chosen direction index, sample `num_samples` random latents and
    interpolate each along the same direction over `steps` magnitudes.
    """
    ...
    # Sample multiple random latents
    z_samples = torch.randn(num_samples, *dm.sample_dim[1:], device=dm.device)
    mags = torch.linspace(-max_mag, max_mag, steps, device=dm.device)
    # Create grid: rows=input latents, cols=magnitudes
    fig, axes = plt.subplots(num_samples, steps, figsize=(2 * steps, 2 * num_samples))

    for s in range(num_samples):
        z = z_samples[s : s + 1]
        for i, mag in enumerate(mags):
            with torch.no_grad():
                mag_batch = mag.view(1)
                img_latent = dm.custom_diffusion.generate_image(
                    z,
                    direction_idx=direction_idx,
                    magnitude=mag_batch,
            ...
            # Convert latent to image [0..1] HWC
            img = ((img_latent[0] / 2 + 0.5).clamp(0, 1)).permute(1, 2, 0).cpu().numpy()

            ax = axes[s, i] if num_samples > 1 else axes[i]
            ax.imshow(img)
            ax.set_title(f"mag={mag:.2f}", fontsize=6)
            ax.axis("off")
